...
unshortenBtn.addEventListener("click", async () => {
  ...
  if (!isShortUrl) {
    alert("Please enter a valid shortened URL (e.g., https://www.shorten-url-api.infobrains.club/shorten/cm8awwwq800ves62i0yj106cx)")
    return
  }
  ...
  try {
    ...
    if (response.status === 500) {
      ...
      shortenResult.innerHTML = `<p>Original URL: <a href="${originalUrl}" target="_blank">${originalUrl}</a></p>`
    }
  } catch (error) {
